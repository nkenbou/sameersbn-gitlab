...
## Git を使うにあたっての問題点 (個人的な予想)

1プロジェクトでリポジトリを2つにわけたほうがよさそう…

* プロジェクト管理資料、一時的な資料、(Office 製品で作った設計書)
* ソースコード、自動テスト
...
* ファイルのロックができない
  * ファイルのマージできない (しにくい) バイナリや Office 製品で作った資料の運用をどうするか
* 分散型バージョン管理システムは大きいサイズのファイルの扱いが苦手と言われている
  * ただし、明確な説明がある情報は見つけられなかった。うわさ話レベル。
* 履歴が汚れる
  * ブランチを作ったり、履歴を意識しながら実装を進めていくので、ドキュメント編集の履歴が混ざると…

というか、資料類は今までどおり Subversion でもよいと思ってもいる
